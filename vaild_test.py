...
if __name__ == "__main__":
    ...
    for numb, information in enumerate(dataset_information):
        ...
        model = ADTransformer(block_size, num_layers=8, heads= information[3] , device=device).to(device)
        model.eval()

        for i in range(1, 2):
            state_dict = torch.load('./Model/Transformer_Base_annthyroid_21feat_normalised.pt')
